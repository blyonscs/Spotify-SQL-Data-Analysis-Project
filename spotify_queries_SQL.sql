...
-- Some songs dont have a video to get views from and some dont stream so we won't remove them
SELECT *
FROM spotify 
WHERE track = NULL
OR artist = NULL
...
OR album = NULL
OR comments = NULL
OR duration_min = NULL
OR duration_min <= 0 
OR title = NULL;

-- two songs are returned with duration_min = 0 so we will remove them
...
------------------------------------
-- Find the cumulative sum of likes for tracks ordered by the number of views using window functions
------------------------------------
